# When agents take actions in the real world, 
# we need robust safety mechanisms. 
# Let’s explore patterns for building safe action systems using a calendar coordination example.
...
# For example, scheduling a meeting might involve checking availability, creating a calendar event, and sending notifications.
# Transaction management lets us treat these related actions as a single unit that either completely succeeds or completely fails.

# The transaction pattern becomes particularly powerful when combined with reversible actions.
# If any step in a multi-step process fails, we can automatically roll back all previous steps. This prevents inconsistent states where, for example,
# a calendar event exists but notifications weren’t sent, or notifications were sent but the event wasn’t created. It’s like having an “undo” button for entire sequences of operations.

# Here is a simple transaction management system that we might use to collect the actions that an agent is
# executing during the agent loop and then either commit or rollback all of them at the end of the loop.
...
# Sometimes we want more control than just automatic rollback on failure. Staged execution allows us to plan out a series of actions, 
# review them for safety and correctness, and only then execute them. 
# This is particularly valuable when some actions are more reversible than others, or when we want to give a human or more capable AI system a chance to review plans before execution.

# The review phase can catch problems that might not be apparent when looking at individual actions.
# For example, while each step in setting up a large meeting might be valid on its own, 
# a reviewer might notice that the combination of actions would overwhelm a team’s schedule or violate company policies about meeting sizes.
# This pattern also allows for optimization - a reviewer might notice more efficient ways to accomplish the same goal.
...
# At first glance, having many small, focused tools seems like good design - 
# it follows the Unix philosophy of “do one thing and do it well.
# ” However, when it comes to agent safety, this approach can backfire. 
# Small, focused tools often lack the context to make safe decisions, and they put the burden of correct orchestration on the agent.

# A single comprehensive tool, in contrast, can encapsulate not just the mechanics of an operation but also the business rules and safety constraints that make it correct. 
# Instead of hoping the agent will check availability before scheduling a meeting, 
# verify attendees before sending invites, and handle timezone conversions correctly, 
# we can build all of these safety checks into a single tool. 
# This makes it much harder for the agent to accidentally create problematic situations, even if it tries to use the tool in unexpected ways.
...
# The single tool encapsulates the business logic of what makes a “safe” meeting scheduling operation, 
# making it much harder for the agent to create problematic situations.