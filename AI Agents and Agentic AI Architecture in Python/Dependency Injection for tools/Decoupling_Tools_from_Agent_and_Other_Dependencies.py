...
# In the lessons, we have been writing tools that use an architectural concept that we haven’t discussed yet, the action_context. 
# This pattern is helpful for supporting dependency injection and decoupling tools from the agent and other dependencies. 
# Let’s explore how the action_context pattern works and why it’s valuable for building flexible and maintainable agent architectures.

# Let’s take a look at an example of why we need dependency injection and how the action_context pattern can help. 
# Consider building an AI agent that helps develop and review code. In a typical workflow, the agent first acts as an expert developer to write code based on requirements. Later, 
# we want the agent to switch roles and act as a code reviewer, critiquing its own work. This seems straightforward at first, but it surfaces a crucial architectural challenge.

# For the code review to be effective, the LLM needs to understand the full context of how the code was developed. 
# What requirements were specified? What constraints were discussed? What alternatives were considered and rejected?

# Let’s assume that all of this information exists in the conversation history stored in the agent’s memory. 
# What we would like to do is allow the LLM to see the memory of what happened in the agent loop, which contains a history of the code generation and requirements, and use it to provide a more informed code review. 
# Let’s attempt to implement this code review tool:
...
# We face an immediate problem. Our tool needs access to the conversation history in the memory, 
# but we can’t simply import the agent instance or directly access its memory. 
# That would create tight coupling between the tools and the agent implementation, making our tools less reusable and harder to test.

# What we need is a way to provide the tool with access to memory and other resources without creating direct dependencies. 
# This is where the ActionContext pattern comes in.
...
# Now we can modify our tool to accept an ActionContext, 
# allowing it to access the conversation history without knowing where it comes from:
...
# Consider a tool that needs to update a project management system with the status of code reviews. This requires authentication, 
# but we don’t want to hardcode credentials or configure them at the tool level.
